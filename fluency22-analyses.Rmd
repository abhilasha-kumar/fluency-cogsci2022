...
```{r}
fluency_data = read.csv("data/precomputed_data.csv") %>% filter(domain == "animals")
```
...
ordermodel = lmer(data = order_model_data, 
     value ~ response_number*similarity + 
              ...
              control=lmerControl(check.conv.singular = .makeCC(action = "ignore",  tol = 1e-4),
                                 optimizer = "bobyqa"))
summary(ordermodel)
sjPlot::plot_model(ordermodel, type = "int")
```
...
summarise(ci = list(mean_cl_boot(value) %>% 
  ...
  theme_few()+
  scale_fill_few()+
      scale_y_continuous(breaks = seq(0, 0.1, 0.04), limits = c(0, 0.1))+
...
fig = fluency_data[c(792:797, 209:214),]%>%
  ...
  select(subject, response_number, response, norm_phon, word2vec_similarity, 
...
fluency_data_fits <- read_csv("newdata-fullfits.csv")
...
participanttestdata = participant_errors %>% filter(errortype == "optimal") %>%
  mutate(k = ifelse(type %in% c("static", "dynamic"), 2, 3),
                  BIC = k*log(number_of_items) - 2*(LL))

```

We perform the sign test for model fits. 

```{r}
## sign test

m_troyer = modeltestdata %>% filter(!norms %in% c("simdrop")) %>%
  select(dataset, subject, type, BIC) %>%
  pivot_wider(names_from = type, values_from = BIC) %>%
  mutate(pmodels_dynamic = as.numeric(plocaldynamic < dynamic   | pglobaldynamic < dynamic  | 
                                        pswitchonlydynamic < dynamic ))

binom.test(sum(m_troyer$pmodels_dynamic),nrow(m_troyer), alternative = "greater")


m_simdrop = modeltestdata %>% filter(!norms %in% c("troyer")) %>%
  select(dataset, subject, type, BIC) %>%
  pivot_wider(names_from = type, values_from = BIC) %>%
  mutate(pmodels_dynamic = as.numeric(plocaldynamic < dynamic   | pglobaldynamic < dynamic | 
                                        pswitchonlydynamic < dynamic ))
...
m_participant = participanttestdata %>% 
  select(dataset, subject, type, BIC) %>%
  pivot_wider(names_from = type, values_from = BIC) %>%
  mutate(pmodels_dynamic = as.numeric(plocaldynamic < dynamic   | pglobaldynamic < dynamic | 
                                        pswitchonlydynamic < dynamic ))
...
bestmodel = modeltestdata %>%
#filter(!norms %in% c("troyer", "jointsimdrop")) %>%
  group_by(subject, dataset) %>%
  slice_min(BIC) %>%
  group_by(dataset,type) %>%
  summarize(count = n()) 

data_sum = bestmodel %>% group_by(dataset) %>% summarise(sum = sum(count))
bestmodel = bestmodel %>% left_join(data_sum) %>% 
...
This section goes over how the list of animals used in the foraging models was created. Please refer to the Jupyter notebook for more details about the final stimuli.
